...
#   hubot jira save filter <JQL> as <name> - Save JIRA JQL query as filter in the brain
#   hubot jira use filter <name> - Use a JIRA filter from the brain
#   hubot jira show filter(s) - Show all JIRA filters
#   hubot jira show filter <name> - Show a specific JIRA filter
#   hubot jira 이번주기 - 이번주기 이슈 목록
#   hubot jira 이번주기할일 - 이번주기 이슈 중 완료되지 않은 이슈 목록
#   hubot jira <project>할일 - <project>의 이슈 중 완료되지 않은 이슈 목록
#
# Author:
#   codec
...
module.exports = (robot) ->
  ...
  search = (msg, jql, cb) ->
    get msg, "search/?jql=#{escape(jql)}", (result) ->
      ...
      resultText = "#{result.total}개의 이슈를 찾았습니다. #{process.env.HUBOT_JIRA_URL}/secure/IssueNavigator.jspa?reset=true&jqlQuery=#{escape(jql)}"
      if result.issues.length <= maxlist
        ...
        cb resultText + " (너무 많아서 나열하지 않겠습니다)"
  ...
  robot.respond /jira 이번\s*주기$/i, (msg) ->
    if msg.message.user.id is robot.name
      return
      
    search msg, "sprint in openSprints()", (text) ->
      msg.reply text
  
  robot.respond /jira 이번\s*주기\s*할\s*일$/i, (msg) ->
    if msg.message.user.id is robot.name
      return
      
    search msg, "sprint in openSprints() and resolution = unresolved", (text) ->
      msg.reply text

  robot.respond /jira (.*)\s*할\s*일/i, (msg) ->
    if msg.message.user.id is robot.name
      return
      
    search msg, "project = " + msg.match[1] + " and resolution = unresolved", (text) ->
      msg.reply text
  ...
  robot.respond /jira save filter (.*) as (.*)/i, (msg) ->
  ...
  robot.respond /jira delete filter (.*)/i, (msg) ->
    filters.delete msg.match[1]

  robot.respond /jira (use )?filter (.*)/i, (msg) ->
    name    = msg.match[2]
  ...
  robot.respond /jira (show )?filter(s)? ?(.*)?/i, (msg) ->
